...
const CircularText = ({
  ...
  return (
    <motion.div
      initial={{ rotate: 10 }}
      className={`rounded-full w-full h-full text-white font-black text-center cursor-pointer ${className}`}
      animate={controls}
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span
            key={i}
            className="absolute rounded-full inline-block inset-0 transition-all duration-500 ease-[cubic-bezier(0,0,0,1)]"
