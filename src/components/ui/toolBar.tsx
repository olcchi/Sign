  ...
  return (
    <MotionConfig transition={transition}>
      <div className="absolute bottom-2 right-2" ref={ref}>
        <div className="h-full w-full rounded-xl border border-zinc-700 bg-zinc-800">
          ...
          <div className="flex space-x-2 p-2" ref={menuRef}>
            {ITEMS.map((item) => (
              <button
                ...
                className={cn(
                  "relative flex h-9 w-9 shrink-0 scale-100 select-none appearance-none items-center justify-center rounded-lg text-zinc-500 transition-colors hover:bg-zinc-600 hover:text-zinc-800 focus-visible:ring-2 active:scale-[0.98]",
                  active === item.id ? "bg-zinc-600 text-zinc-800" : ""
                )}
                type="button"
                onClick={() => {
